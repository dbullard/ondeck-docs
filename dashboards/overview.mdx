...
The Setup Wizard is also available from here and from the `File` menu on Mac or the ellipsis menu on iOS (Coming Soon), so you can rerun guided setup whenever you add new devices or services.
...
The Variables window lets you define a value once and use it across text inputs and automations, instead of retyping it in each place.

In Live Mode, Relay suggests wireless telemetry variables automatically. They are labeled with the channel names you saved for your devices, so they are easy to recognize when you pick one.
