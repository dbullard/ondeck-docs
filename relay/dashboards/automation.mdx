...
- **Triggers** that start a workflow — see [Trigger Nodes](/relay/dashboards/nodes/triggers)
- **Sources** that feed data into the graph, like Current Time or an imported console or wireless channel name — see [Source Nodes](/relay/dashboards/nodes/sources)
- **Actions and destinations** that do something — see [Action & Destination Nodes](/relay/dashboards/nodes/actions)
- **Utilities** like delays, repeaters, variables, branching, toggles, counters, sequences, latches, and merges — see [Utility Nodes](/relay/dashboards/nodes/utilities)
...
Values you define in the Variables window are available to automations as well as text inputs, so one change stays consistent across your graphs and dashboard. See [Shared values and variables](/relay/dashboards/variables).
